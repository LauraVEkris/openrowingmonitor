...
export const DASHBOARD_METRICS = {
  distance: {
    ...
    template: (metrics, config) => {
      const distance = metrics?.sessiontype === 'distance' ? Math.max(metrics?.intervalTargetDistance - metrics?.intervalLinearDistance, 0) : (metrics?.sessiontype === 'rest' && metrics?.pauseCountdownTime > 0 ? undefined : metrics?.totalLinearDistance)
